            <!-- Barra de navegação lateral -->
            <div class="coluna-barra-lateral d-none d-sm-block bg-inscricao">
                <div class="container-fluid" id="BotoesBarraLateral">
                    <img class="pt-3 pb-3" src="/assets/img/logo-pes-circular.png" width="100%">
                    <button class="btn btn-verde w-100 mt-2">Ver/atualizar cadastro</button>
                    <button class="btn btn-verde w-100 mt-2">Marcar entrevista</button>
                    <button class="btn btn-verde w-100 mt-2">Consultar etapas</button>
                    <button class="btn btn-verde w-100 mt-2">Ver nota da prova/redação</button>
                    <button class="btn btn-verde w-100 mt-2">Ver flags durante aula teste</button>
                    <button class="btn btn-verde w-100 mt-2">Enviar justificativa de falta</button>
                    <button class="btn btn-verde w-100 mt-2">Enviar dúvidas</button>
                    <button class="btn btn-verde w-100 mt-2 mb-3">Ver resultado PS</button>
                </div>
            </div><!-- /Barra de navegação lateral -->
            <!-- Coluna do formulário -->
            <div class="coluna-formulario container-fluid text-center">
                <h3 class="mt-5">As ações de cada opção ao lado serão apresentadas aqui.</h3>
                <form action="/logout" method="POST">
                    <button class="btn btn-bordo mt-3" type="submit">Logout</button>
                </form>
            </div><!-- /Coluna do formulário -->